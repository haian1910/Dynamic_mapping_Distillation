...
class STSModel_Qwen(nn.Module):
    """Wrapper for STS (Semantic Textual Similarity) tasks using a base model"""
    def __init__(self, base_model):
        super(STSModel_Qwen, self).__init__()
        self.base_model = base_model
        self.config = base_model.config  # Expose config for save_pretrained
...
class Distiller(nn.Module):
    ...
    def load_teacher_model(self):
        ...
        config = AutoConfig.from_pretrained(
            "Qwen/Qwen3-Embedding-0.6B",  # Changed to match adapter_config.json
            trust_remote_code=True
        )
        config.is_model_parallel = False
        
        # Load tokenizer
        tokenizer = self.load_tokenizer("Qwen/Qwen3-Embedding-0.6B")  # Changed to match base model
        ...
        teacher_model = STSModel_Qwen(model)
